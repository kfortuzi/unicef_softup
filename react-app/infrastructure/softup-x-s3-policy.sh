...
cloud_front_ouput=$(stack_output softup-x-cloudfront-$ENVIRONMENT)
...
aws cloudformation deploy \
  --region $AWS_REGION \
  --stack-name softup-x-s3-policy-$ENVIRONMENT \
  --template-file stacks/softup-x-s3-policy-stack.yaml \
