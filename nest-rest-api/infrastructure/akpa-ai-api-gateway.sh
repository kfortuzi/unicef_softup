...
api_lambda_output=$(stack_output akpa-ai-api-lambda-$ENVIRONMENT)
api_lambda_arn=$(param_from_stack_output "$api_lambda_output" LambdaARN)

aws cloudformation deploy \
  --region $AWS_REGION \
  --stack-name akpa-ai-api-gateway-$ENVIRONMENT \
  --capabilities CAPABILITY_NAMED_IAM \
  --template-file stacks/akpa-ai-api-gateway-stack.yaml \
